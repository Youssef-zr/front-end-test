...
(function(global, factory) {
    ...
    if (isCallable(result.then)) {
      return result.then(function(values) {
        ...
        return {
          ...
          errors: allValid
            ? []
            : [this$1._createFieldError(field, rule, data, targetName)]
        };
      });
    }
  ...
  VeeValidate$1.use = function use(plugin, options) {
    ...
    plugin(
      { Validator: Validator, ErrorBag: ErrorBag, Rules: Validator.rules },
      options
    );
  };
  ...
  var messages = {
    ...
    length: function(field, ref) {
      ...
      if (max) {
        return (
          "The " + field + " length must be between " + length + " and " + max
        );
      }
